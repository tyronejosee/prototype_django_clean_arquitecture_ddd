...
from modules.orders.domain.exceptions import OrderNotFoundError, OrderDomainError
...
def test_get_order_detail_successfully(user_client: APIClient) -> None:
    ...
    with patch(
        "modules.orders.presentation.controllers."
        "order_detail_controller.get_get_order_use_case"
    ) as mock_provider:
        mock_use_case = Mock()
...
def test_delete_order_successfully(user_client: APIClient) -> None:
    ...
    with patch(
        "modules.orders.presentation.controllers."
        "order_detail_controller.get_cancel_order_use_case"
    ) as mock_provider:
        mock_use_case = Mock()
        mock_use_case.execute.return_value = order
        mock_provider.return_value = mock_use_case
...
def test_delete_order_with_domain_error_returns_400(user_client: APIClient) -> None:
    ...
    with patch(
        ...
        mock_use_case = Mock()
        mock_use_case.execute.side_effect = OrderDomainError(
