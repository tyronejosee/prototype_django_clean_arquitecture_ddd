...
from modules.orders.infrastructure.models import OrderModel, OrderItemModel
...
def test_create_order_successfully(user_client: APIClient) -> None:
    ...
    with (
        patch(
            "modules.orders.application.providers.get_cart_repository"
        ) as mock_cart_repo,
        patch(
            "modules.orders.application.providers.get_product_repository"
        ) as mock_product_repo,
    ):
