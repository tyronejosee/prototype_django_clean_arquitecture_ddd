...
class CouponDiscountHandler(BaseDiscountHandler):
    ...
    def apply(self, order) -> DiscountResult:
        ...
        discount = order.total() * coupon.discount_percent.value  # type: ignore[operator]
        final_price = order.total() - discount  # type: ignore[operator]

        return DiscountResult(final_price=final_price, applied_discounts=[f"Coupon {coupon.code.value}: -{discount}"])
