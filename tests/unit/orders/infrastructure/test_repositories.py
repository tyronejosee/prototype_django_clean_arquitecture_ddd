...
def test_update_order_status_successful() -> None:
    ...
    user_id = uuid4()
    order_model = OrderModel.objects.create(user_id=user_id, status=OrderStatus.PENDING.value)
    repo = OrderRepository()
...
def test_cancel_order_sets_status_to_cancelled() -> None:
    ...
    order_model = OrderModel.objects.create(user_id=uuid4(), status=OrderStatus.PAID.value)
    repo = OrderRepository()
