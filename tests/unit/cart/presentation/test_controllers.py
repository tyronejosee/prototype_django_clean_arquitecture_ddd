...
def test_cart_get_success(fake_user: Mock, fake_cart: Cart) -> None:
    ...
    with patch(
        "src.modules.cart.presentation.controllers.cart_controller.get_get_cart_use_case"
    ) as mock_provider:
        use_case = Mock()
...
def test_cart_get_not_found(fake_user: Mock) -> None:
    ...
    assert response.data["detail"] == "Cart not found."  # type: ignore[union-attr]
...
def test_cart_post_success(fake_user: Mock, fake_cart: Cart) -> None:
    ...
    with patch(
        "src.modules.cart.presentation.controllers.cart_controller.get_create_cart_use_case"
    ) as mock_provider:
        use_case = Mock()
        use_case.execute.return_value = fake_cart
        mock_provider.return_value = use_case
    ...
    assert response.data["user_id"] == str(fake_user.id)  # type: ignore[union-attr]
...
def test_cart_post_invalid_domain(fake_user: Mock) -> None:
    ...
    assert response.data["detail"] == "Cart already exists."  # type: ignore[union-attr]
...
def test_cart_put_success(fake_user: Mock, fake_cart: Cart) -> None:
    ...
    with patch(
        "src.modules.cart.presentation.controllers.cart_controller.get_update_cart_use_case"
    ) as mock_provider:
        use_case = Mock()
        use_case.execute.return_value = fake_cart
        mock_provider.return_value = use_case
...
def test_patch_cart_item_success(
    ...
    with patch(
        "src.modules.cart.presentation.controllers.cart_item_controller.get_patch_cart_item_use_case"
    ) as mock_provider:
        use_case = Mock()
        use_case.execute.return_value = fake_cart
        mock_provider.return_value = use_case
...
def test_patch_cart_item_not_found(fake_user: Mock, item_id: UUID) -> None:
    ...
    with patch(
        "src.modules.cart.presentation.controllers."
    ...
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.data["detail"] == "Item not found."  # type: ignore[union-attr]
...
def test_patch_cart_item_invalid_quantity(fake_user: Mock, item_id: UUID):
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.data["detail"] == "Quantity must be between 1 and 10."  # type: ignore[union-attr]
