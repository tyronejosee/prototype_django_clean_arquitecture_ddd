...
from apps.cart.domain.exceptions import CartDomainError, CartNotFoundError
from apps.cart.presentation.serializers.cart_serializer import CartSerializer
...
class CartController(APIView):
    ...
    def put(self, request: Request) -> Response:
        ...
        use_case = get_update_cart_use_case()
        try:
            data: dict = serializer.validated_data  # type: ignore[arg-type]
            data["user_id"] = request.user.id
            cart = use_case.execute(data)
